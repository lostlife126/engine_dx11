...
bool StaticMesh::m_loadMS3DFile(const wchar_t* Filename)
{
...
bool StaticMesh::m_LoadTextures(const wchar_t* textureFilename)
{
	HRESULT hr = D3DX11CreateShaderResourceViewFromFileW(m_render->m_pd3dDevice, textureFilename, NULL, NULL, &m_texture, NULL);
	if (FAILED(hr))
		return false;
...
bool StaticMesh::m_InitShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
