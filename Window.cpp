...
namespace MyEngine
{
		...
		m_isresize(false)
	{
		if (!m_wndthis)
			...
			Log::Get()->Error("Window уже был создан");
	}
	...
	bool Window::Create(const DescWindow& desc)
	{
		...
		{
			Log::Get()->Error("Не удалось зарегистрировать окно");
			return false;
		}
	...
	LRESULT CALLBACK MyEngine::StaticWndProc(HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
	{
